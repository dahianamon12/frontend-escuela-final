...
export class HorarioListComponent implements AfterViewInit {
  ...
  private open(data: HorarioDialogData): void {
    this.dialog
      .open(HorarioDialogComponent, { width: '480px', data })
      .afterClosed()
      .pipe(filter(Boolean))
  ...
  eliminar(row: HorarioRead): void {
    ...
    this.svc.delete(row.id_horario).subscribe({
      next: () => {
        this.snack.open('Horario eliminado', 'OK', { duration: 3000 });
        this.reload();
      },
      error: (err: HttpErrorResponse) =>
        this.snack.open(this.msg(err), 'Cerrar', { duration: 6000 }),
